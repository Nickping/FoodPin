import UIKit
import CoreData
class RestaurantTableViewController: UITableViewController,newRestaurantItem, NSFetchedResultsControllerDelegate, UISearchResultsUpdating {
    func updateSearchResults(for searchController: UISearchController) {
        print("upateSearchResults")
    ...
    override func viewDidLoad() {
        ...
        navigationController?.navigationBar.prefersLargeTitles = true // largee bar
        
        // MARK: navigationbar controll
        //navigationController?.navigationBar.setBackgroundImage(UIImage(), for: .default) //네비게이션바 투명
        //navigationController?.navigationBar.shadowImage = UIImage()
        navigationController?.hidesBarsOnSwipe = true //스와이프 할때 네비게이션바 지움
        
        if let customFont = UIFont(name: "Rubik-Medium", size: 40.0) {
            navigationController?.navigationBar.largeTitleTextAttributes =
                [ NSAttributedStringKey.foregroundColor: UIColor(red: 231.0/255.0, green: 76.0/255.0, blue: 60.0/255.0, alpha: 1.0), NSAttributedStringKey.font: customFont ]
        }
    ...
    func filterContent(for searchText : String){
        ...
        searchResult = restaurants.filter({ (restaurant) -> Bool in
            if let name : String = restaurant.name{
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if searchController.isActive{
            print("numberOfRowsInSection Active" + "\(searchResult.count)")
            return searchResult.count
        }else{
            print("numberOfRowsInSection not Active" + "\(restaurants.count)")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if searchController.isActive {
            print("search Controller is active now")
            print(indexPath.row)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "showRestaurantDetail" {
            if let indexPath = tableView.indexPathForSelectedRow {
                ...
                let restaurant = (searchController.isActive) ? self.searchResult[indexPath.row]: restaurants[indexPath.row]
                
                destinationController.restaurant = restaurant
            }
        }
        else if segue.identifier == "addRestaurant"{
            var topVc  = segue.destination as! UINavigationController
            var nextVC = topVc.topViewController as! NewRestaurantTableViewController
    ...
    func newRestaurantCreated(name: String, type: String, location: String, phone: String, description: String, image: String, isVisited: Bool) {
        let newRestaurant = Restaurant(name: name, type: type, location: location, phone: phone, description: description, image: image, isVisited: isVisited)
